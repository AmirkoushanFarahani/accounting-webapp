...
function PaymentCreate({
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (overAllocated) {
      setError("جمع تخصیص‌ها نمی‌تواند بیشتر از مبلغ دریافت باشد.");
      return;
    }
    const form = new FormData(event.currentTarget);
    try {
...
function PaymentPost({
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const receivable = String(form.get("receivable"));
    if (cash === receivable) {
      setError("حساب نقد/بانک و حساب دریافتنی باید متفاوت باشند.");
      return;
    }
    setBusy(true);
    setError("");
